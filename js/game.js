...
function Tiles(game) {
	this.theGame = game; //You lost it
	this.tileNames = {blank: 0, icecream: 1};
	this.tiles = [createTileImage("../img/tile.png"), createTileImage("../img/icecream.png")];
	this.selectedBuilding = null;
}

Tiles.prototype.selectTile = function(tileID) {
	if(tileID >= this.tiles.length) {
		tileID = this.tileNames.blank;
	}
	this.selectedTile = tileID;
}

Tiles.prototype.getTileImage = function(tileID) {
...
function Game(canvas, game, gridSizeW, gridSizeH) {
	...
	this.tile = this.tiles.getTileImage(this.tiles.tileNames.blank)
...
Game.prototype.handleMouseDown = function(e) {
	...
	switch (Tools.current) {
		case Tools.BUILD:
			var matrix = this.translatePixelsToMatrix(e.clientX,e.clientY) 
			this.tileMap[matrix.row] = (this.tileMap[matrix.row] === undefined) ? [] : this.tileMap[matrix.row];
			console.log("Selected tile is: " + this.tiles.selectedTile)
			this.tileMap[matrix.row][matrix.col] = this.tiles.selectedTile;
			break;
		case Tools.MOVE:
	...
	for (var row = startRow; row < rowCount; row++) {
		for (var col = startCol; col < colCount; col++) {
			...
			if (this.tileMap[row] != null && this.tileMap[row][col] != null) {
				// Place building
				var tileImage = this.tiles.getTileImage(this.tileMap[row][col]);
				ypos -= tileImage.height - this.tile.height;
				xpos -= (tileImage.width / 2) - (this.tile.width / 2);
				this.c.drawImage(tileImage,Math.round(xpos),Math.round(ypos),tileImage.width,tileImage.height);
			} else {
